...
const RecipeIndexPage = () => {
	...
	const recipes = useMemo(() => {
		...
		const getAge = (ages.includes(umur.toString()) ? umur : "semua")
			.toString()
			.toLowerCase();
			...
			<div className="container min-h-screen m-auto px-4 flex flex-col space-y-8 py-4 lg:px-12 xl:px-24">
				<div className="text-center">
					<h1 className="text-[#3056D3] font-bold text-base">
						Refrensi Resep MPASI
					</h1>
					<h2 className="font-bold text-2xl mt-1 mb-3 md:text-3xl">
						Moms and Dad penasaran?{" "}
					</h2>
					<p className="mx-auto text-[#637381] text-sm md:w-3/4 md:text-base xl:w-1/2">
						{" "}
						menu MPASI apa sih yang cocok diberikan kepada kecil supaya ga
						bosan? Yuk simak resep MPASI berikut ini
					</p>
				</div>
				...
				<div className="grid  grid-cols-2 gap-3 md:grid-cols-4">
					{ages.map((age, k) => (
						<Button
							className={`${
								age.toLowerCase() == filteredAge
									? "bg-primary-2"
									: "bg-primary-1"
							} duration-500 hover:bg-primary-2`}
							key={k}
							onClick={() => filterAge(age)}>
							{age} {age != "Semua" ? "bulan" : "usia"}{" "}
							{age == "12" && "ke atas"}
						</Button>
					))}
				</div>
							...
							<Card>
								...
								<div className="line-clamp-2">
									{recipe.bahan.map((bahan, k) => (
										<span key={k}>
											{bahan}{" "}
											<span className="inline-block px-xs text-gray-400">
												•
											</span>{" "}
										</span>
									))}
