...
const RecipeDetailPage = () => {
	...
	useEffect(() => {
		const findRecipe = recipeJson.find(
			(recipe) => recipe.nama == (route.query.name ?? "")
		);
	...
	useEffect(() => {
		...
		if (recipe) {
			...
			const recommendation: RecomendationArticles[] = recipeJson
				.filter(
					(recipeData) =>
						recipeData.Usia == recipe.Usia &&
						recipeData.nama != route.query.name
				)
				.map((recipe) => ({
					title: recipe.nama,
						...
						{!recipe ? (
							...
							<div className="">
								<Breadcrumb className="mb-6">
									...
									<Breadcrumb.Item>
										<Link
											href={`/resep?umur=${recipe.Usia}`}
											className="text-black flex items-center gap-x-2">
											{recipe.Usia} {recipe.Usia != "Semua" ? "bulan" : "usia"}{" "}
											{recipe.Usia == "12" && "ke atas"}
										</Link>
									</Breadcrumb.Item>
								...
								<ArticleLayout
									...
									<div className="border border-gray-300 rounded-md px-5 py-6 grid grid-cols-2 gap-y-3 text-center lg:grid-cols-3">
										...
										<div className="">
											...
											<p>
												{recipe.Usia}{" "}
												{recipe.Usia != "Semua" ? "bulan" : "usia"}{" "}
												{recipe.Usia == "12" && "ke atas"}
											</p>
										</div>
									</div>

									<h1 className="font-bold text-3xl text-teal-500">Bahan</h1>
									...
									{recipe.bumbu_halus && (
										<>
											<h1 className="font-bold text-3xl">Bumbu Halus</h1>
											<ul className="list-disc pl-5 space-y-2">
									...
									<h1 className="font-bold text-3xl text-teal-500 pt-5">
										Nutrition Facts{" "}
										<span className="text-sm font-normal">(per sajian)</span>
									</h1>
