...
export const validationDto = yup.object({
	...
	weight: yup
		.number()
		.positive("Berat badan wajib diisi")
		.required("Berat badan wajib diisi"),
	height: yup
		.number()
		.positive("Tinggi badan wajib diisi")
		.required("Tinggi wajib diisi"),
	gender: yup
		.string()
		.oneOf(["laki-laki", "perempuan"], "Jenis kelamin wajib dipilih")
...
export const NutritionBox = (props: NutritionBoxData) => {
	return (
		<div className="bg-white shadow-lg w-24 h-24 rounded-md flex flex-col justify-end pb-5 relative md:w-28 lg:w-32 lg:h-28">
			<props.Icon className="text-4xl absolute -top-5 left-1/2 transform -translate-x-1/2 md:text-5xl md:-top-7 xl:text-6xl xl:-top-8" />
...
export const InterpretationLabelBtn = ({
	status,
	hex,
}: InterpretationLabelBtnData) => {
	return (
		<div className="flex flex-row items-center">
			<h3 className="text-lg font-bold md:text-xl">Interpretasi</h3>
			<span
				className="ml-3 font-bold px-4 py-2 rounded-md text-white text-xs md:text-sm md:px-5"
